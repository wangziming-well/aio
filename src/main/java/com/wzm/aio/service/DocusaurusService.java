...
import com.wzm.aio.util.GitUtils;
...
public class DocusaurusService {
    ...
    private void checkEnv() {
        if (!hasNpm() || !hasNodejs())
            throw new RuntimeException("docusaurus环境未就绪，请检查npm和nodejs");
    }
    ...
    private void loadNote() {
        ...
        GitUtils.cloneOrPull(noteRepoUrl, workDirectory, this.commandOutputConsumer);
        //将note移动到Docusaurus项目中
        String repoName = GitUtils.analysisRepoName(noteRepoUrl);
    ...
    private void loadNotePicture() {
        ...
        GitUtils.cloneOrPull(repoUrl, workDirectory, this.commandOutputConsumer);
        //将仓库中图片文件夹中的所有图片放到静态映射文件夹中
        String imgLocation = properties.getNotePicture().getImgLocation();
        String repoName = GitUtils.analysisRepoName(repoUrl);
    ...
    private void installPluginDependencies() {
        ...
        for (String pluginDependence : pluginDependencies) {
            ...
            consumeCommand(command);
            Shell.exec(this.commandOutputConsumer,this.projectContext,command);
        }
    }
    ...
    private boolean pluginDependenceExists(String dependenceName){
        ...
        consumeCommand(command);
        String output = Shell.execStr(this.projectContext, command);
        consumeCommandOutput(output);
    ...
    private void loadDocusaurusStaticPages() {
        ...
        String buildOutDir = Path.of(properties.getStaticLocation(), project.getStaticPath()).toString();
